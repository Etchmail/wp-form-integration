<?php
/**  Etchmail × Formidable – editor-panel markup
 *
...
$emfi_compat = [ // todo
	...
	'textarea'   => [ 'textarea' ],
	'select'     => [ 'dropdown' ],
	'checkbox'   => [ 'multiselect','consentcheckbox' ],
							...
							<div class="etchmail-mapping-row">
								...
								<div class="etchmail-mapping-col">
									<select name="emfi_field_map[<?php echo esc_attr( $tag->name ); ?>]"
									        ...
									        data-frm-field="<?php echo esc_attr( $tag->name ); ?>">
...
<script>
    jQuery(function ($) {
        ...
        function misConfigCheck(mapped, forms, lists) {
            ...
            Object.entries(mapped).forEach(([frm, etch]) => {
                if (!formNames.has(frm))
                    issues.push(`Form field <strong>${frm}</strong> no longer exists in the form.`);
        ...
        $saveBtn.on('click', () => {
            ...
            $('.emfi-map-select').each(function () {
                const cf = $(this).data('frm-field'), dest = $(this).val();
